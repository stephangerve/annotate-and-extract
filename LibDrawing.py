...
def setBBoxesAutomatically(image_w_new_bboxes, boundary, annotate_mode, color, index_number, index_increment, column_pos=None):
    #global x_start, y_start, x_end, y_end
    global last_annotate_mode, last_color
    global drawing
    global label_ready
    global mask_reg_col_line
    global drawing_bbox_outer_boundary
    global im_thresh_otsu
    global annotations
    global bboxes
    global masks
    global header

    x_start = boundary[0][0]
    x_end = boundary[1][0]
    y_start = boundary[0][1]
    y_end = boundary[1][1]
    mask_scan_region = im_thresh_otsu[boundary[0][1]:boundary[1][1], boundary[0][0]:mask_reg_col_line[0][0]]
    # cv2.rectangle(image_w_new_bboxes, (boundary[0][0], boundary[0][1]), (mask_reg_col_line[0][0], boundary[1][1]), BLACK, 1)
    # cv2.imshow("image", image_w_new_bboxes)
    # cv2.waitKey(1)
    min_pix_value, num_pos_y1 = binSearchForBBOXBoundaries("black", mask_scan_region, [i for i in range(y_start, y_end)], scan_up=False)
    min_pix_value, num_pos_y2 = binSearchForBBOXBoundaries("white", mask_scan_region[(num_pos_y1 - y_start):], [i for i in range(num_pos_y1, y_end)], scan_up=False)
    # if len(bboxes) == 0:
    #     top_bbox_y = boundary[0][1]
    # else:
    #     top_bbox_y = list(bboxes.values())[-1][1][1]
    top_bbox_y = boundary[0][1]
    idx_num = 0
    while True:
        min_pix_value, next_num_pos_y1 = binSearchForBBOXBoundaries("black", mask_scan_region[(num_pos_y2 - y_start):], [i for i in range(num_pos_y2, y_end)], scan_up=False)
        if min_pix_value >= 240:
            break
        upper_scan_region = im_thresh_otsu[num_pos_y2:next_num_pos_y1, x_start:x_end]
        cont_white_regs = binSearchContiguousWhiteRegions(upper_scan_region, [i for i in range(num_pos_y2, next_num_pos_y1)], {})
        midpoint_y = findMidpointOfLongestContWhiteReg(cont_white_regs)
        if midpoint_y is None:
            break
        key = generateCode()
        bbox = [(x_start, top_bbox_y), (x_end, midpoint_y)]
        bboxes[key] = bbox
        if annotate_mode == OP_SIMPLE:
            addAnnotation(code=key, shape=SHAPE_BBOX, operation=annotate_mode, column_pos=column_pos, index=index_number)
        elif annotate_mode == OP_COMBINE_W_H:
            addAnnotation(code=key, shape=SHAPE_BBOX, operation=annotate_mode, column_pos=column_pos, index=index_number, grid=True, header=header)
        if odds_only:
            index_number += index_increment + 1
        else:
            index_number += index_increment
        if IMAGE_TYPE == "Exercises":
            mask = [(x_start, top_bbox_y), (mask_reg_col_line[0][0], midpoint_y)]
            masks[key] = mask
            im_region = image_w_new_bboxes[mask[0][1]:mask[1][1], mask[0][0]:mask[1][0]]
            white_rect = np.ones(im_region.shape, dtype=np.uint8) * 150
            masked_reg = cv2.addWeighted(im_region, 0.5, white_rect, 0.5, 1.0)
            image_w_new_bboxes[mask[0][1]:mask[1][1], mask[0][0]:mask[1][0]] = masked_reg
            cv2.rectangle(image_w_new_bboxes, mask[0], mask[1], color, 2)
        cv2.rectangle(image_w_new_bboxes, bbox[0], bbox[1], color, 2)
        x_0, y_0, x_1, y_1 = bbox[0][0], bbox[0][1], bbox[0][0], bbox[0][1]
        l_x_0, l_y_0, l_x_1, l_y_1 = x_0 - 28, y_0, x_1 - 2, y_1 + 20
        cv2.rectangle(image_w_new_bboxes, (l_x_0, l_y_0), (l_x_1, l_y_1), color, -1)
        cv2.putText(image_w_new_bboxes, str(annotations[key]["index"]).zfill(3), (l_x_0 + 2, l_y_0 + 14), cv2.FONT_HERSHEY_SIMPLEX, 0.40, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.imshow("image", image_w_new_bboxes)
        cv2.waitKey(1)
        top_bbox_y = midpoint_y
        min_pix_value, num_pos_y2 = binSearchForBBOXBoundaries("white", mask_scan_region[(next_num_pos_y1 - y_start):], [i for i in range(next_num_pos_y1, y_end)], scan_up=False)
    key = generateCode()
    bbox = [(x_start, top_bbox_y), (x_end, y_end)]
    bboxes[key] = bbox
    if annotate_mode == OP_SIMPLE:
        addAnnotation(code=key, shape=SHAPE_BBOX, operation=annotate_mode, column_pos=column_pos, index=index_number)
    elif annotate_mode == OP_COMBINE_W_H:
    ...
    if odds_only:
        ...
        index_number += index_increment
    if IMAGE_TYPE == "Exercises":
        mask = [(x_start, top_bbox_y), (mask_reg_col_line[0][0], y_end)]
        masks[key] = mask
        im_region = image_w_new_bboxes[mask[0][1]:mask[1][1], mask[0][0]:mask[1][0]]
        white_rect = np.ones(im_region.shape, dtype=np.uint8) * 150
        masked_reg = cv2.addWeighted(im_region, 0.5, white_rect, 0.5, 1.0)
    ...
    cv2.waitKey(1)
    label_ready = False
    drawing_bbox_outer_boundary = True
    image = image_w_new_bboxes
    return image, index_number

def drawBBoxes2(orig_image, image, boundary, command, prev_bboxes, prev_masks, last_image_index, last_annotations, last_header, column_pos=None):
    global x_start, y_start, x_end, y_end
    global label_x_start, label_y_start, label_x_end, label_y_end
    global current_image_index
    global last_annotate_mode
    global drawing
    global drawing_bbox
    global label_ready
    global mask_reg_col_line
    ...
    while drawing:
        ...
        if annotate_mode in BBOX_COLOR.keys():
            ...
            color = RED
        if len(bboxes) > 0:
            unprocessed_reg_boundary = [(boundary[0][0], list(bboxes.values())[-1][1][1]), boundary[1]]
        else:
            unprocessed_reg_boundary = boundary
        if annotate_mode in [OP_SIMPLE, OP_COMBINE_W_H]:
            cv2.setMouseCallback("image", SASOuterBoundaryAndMaskRegion, [unprocessed_reg_boundary])
            cv2.rectangle(image_w_new_bboxes, (x_start, y_start), (x_end, y_end), color, 1)
            cv2.line(image_w_new_bboxes, mask_reg_col_line[0], mask_reg_col_line[1], color, 1)
            if label_ready:
                cv2.rectangle(image_w_new_bboxes, (label_x_start, label_y_start), (label_x_end, label_y_end), color, -1)
                label_str = str(current_image_index).zfill(3)
                font_size = 0.40
                cv2.putText(image_w_new_bboxes, label_str, (label_x_start + 2, label_y_start + 14), cv2.FONT_HERSHEY_SIMPLEX, font_size, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.imshow("image", image_w_new_bboxes)
            cv2.waitKey(1)
            if drawing_bbox_outer_boundary is False:
                image, current_image_index = setBBoxesAutomatically(image_w_new_bboxes, [(x_start, y_start), (x_end, y_end)], annotate_mode, color, current_image_index, 1, column_pos)
                drawing_bbox_outer_boundary = True
        elif annotate_mode in [OP_APP_TO_LAST, OP_SET_HEADER, OP_APP_TO_HEAD]:
            cv2.setMouseCallback("image", scanAndSetBBOXCoordiates, [unprocessed_reg_boundary])
            ...
            if label_ready:
                ...
                if IMAGE_TYPE == "Exercises" and annotate_mode in [OP_SIMPLE, OP_COMBINE_W_H]:
                    ...
                    if mask_y_end >= y_end:
                        im_region = image_w_new_bboxes[mask_y_start:y_end, mask_x_start:mask_x_end]
                        white_rect = np.ones(im_region.shape, dtype=np.uint8) * 150
                        masked_reg = cv2.addWeighted(im_region, 0.5, white_rect, 0.5, 1.0)
                        image_w_new_bboxes[mask_y_start:y_end, mask_x_start:mask_x_end] = masked_reg
                        cv2.rectangle(image_w_new_bboxes, (mask_x_start, mask_y_start), (mask_x_end, y_end), color, 1)
                    else:
                        im_region = image_w_new_bboxes[mask_y_start:mask_y_end, mask_x_start:mask_x_end]
                        white_rect = np.ones(im_region.shape, dtype=np.uint8) * 150
                        masked_reg = cv2.addWeighted(im_region, 0.5, white_rect, 0.5, 1.0)
                        image_w_new_bboxes[mask_y_start:mask_y_end, mask_x_start:mask_x_end] = masked_reg
                        cv2.rectangle(image_w_new_bboxes, (mask_x_start, mask_y_start), (mask_x_end, mask_y_end), color, 1)
            cv2.imshow("image", image_w_new_bboxes)
            cv2.waitKey(1)
            ...
            if drawing_bbox is False:
                ...
                cv2.rectangle(image_w_new_bboxes, (x_start, y_start), (x_end, y_end), color, 2)
                cv2.imshow("image", image_w_new_bboxes)
                cv2.waitKey(1)
                if annotate_mode == OP_APP_TO_LAST:
            ...
            if annotate_mode == OP_APP_TO_LAST:
                ...
                if last_annotate_mode is not None:
                    annotate_mode = last_annotate_mode
            # if len(bboxes) > 0:
            #     condition, image, sc_boundary, sc_annotate_mode = drawSubcolumns3(image_w_new_bboxes, [(list(bboxes.values())[-1][0][0], list(bboxes.values())[-1][1][1]), (boundary[1][0], boundary[1][1])], annotate_mode, grid_ordering, column_pos)
            # else:
            #     condition, image, sc_boundary, sc_annotate_mode = drawSubcolumns3(image_w_new_bboxes, boundary, annotate_mode, grid_ordering, column_pos)
            condition, image, sc_boundary, sc_annotate_mode = drawSubcolumns3(image_w_new_bboxes, unprocessed_reg_boundary, annotate_mode, grid_ordering, column_pos)
        ...
        if keyboard.is_pressed('ctrl+shift+alt+1'):
            ...
            if scan_mode:
                scan_mode = False
                cv2.setMouseCallback("image", setBBOXCoordiates, [unprocessed_reg_boundary])
            else:
                scan_mode = True
                cv2.setMouseCallback("image", scanAndSetBBOXCoordiates, [unprocessed_reg_boundary])
            ...
            if len(bboxes) == 0:
                ...
                if undo_code in masks.keys():
                    masks.pop(undo_code)
                if undo_annotation["shape"] == SHAPE_LINE:
                    if len(annotations) > 0:
                        undo_code = list(annotations.keys())[-1]
                        undo_annotation = annotations.pop(undo_code)
                        bboxes.pop(undo_code)
                        if undo_code in masks.keys():
                            masks.pop(undo_code)
                    if len(annotations) > 0 \
    ...
    return True, image_w_new_bboxes, annotate_mode, bboxes, masks, annotations, current_image_index, header


def drawSubcolumns3(image, boundary, annotate_mode, grid_ordering, column_pos=None):
    global x_start, y_start, x_end, y_end
    ...
    while drawing_grid_outer_boundary:
        ...
        if keyboard.is_pressed('esc'):
            ...
            while keyboard.is_pressed('ctrl+shift+alt+1'):
                continue
            drawing_grid_outer_boundary = False
            return False, image, None, OP_SIMPLE
        elif keyboard.is_pressed('ctrl+shift+alt+2'):
            while keyboard.is_pressed('ctrl+shift+alt+2'):
                continue
            drawing_grid_outer_boundary = False
            return False, image, None, OP_APP_TO_LAST
        elif keyboard.is_pressed('ctrl+shift+alt+3'):
            while keyboard.is_pressed('ctrl+shift+alt+3'):
                continue
            drawing_grid_outer_boundary = False
            return False, image, None, OP_SET_HEADER
        elif keyboard.is_pressed('ctrl+shift+alt+4'):
            while keyboard.is_pressed('ctrl+shift+alt+4'):
                continue
            drawing_grid_outer_boundary = False
            return False, image, None, OP_COMBINE_W_H
        elif keyboard.is_pressed('ctrl+shift+alt+5'):
            while keyboard.is_pressed('ctrl+shift+alt+5'):
                continue
            drawing_grid_outer_boundary = False
            return False, image, None, OP_APP_TO_HEAD
    ...
    while drawing_grid_column_bboxes:
        ...
        if drawing_one_gcb:
            ...
            if IMAGE_TYPE == "Exercises":
                ...
                image_w_gcb[y_start:y_end, mask_x_start:mask_x_end] = masked_reg
                cv2.rectangle(image_w_gcb, (mask_x_start, y_start), (mask_x_end, y_end), color, 1)
                mask_reg_col_lines.append([(mask_x_end, grid_outer_boundary[0][1]), (mask_x_end, grid_outer_boundary[1][1])])
            cv2.imshow("image", image_w_gcb)
            cv2.waitKey(1)
    ...
    idx_increment = len(grid_col_bboxes)
    last_grid_index_numbers = []
    for grid_col_bbox, mask_reg_col_line, i in zip(grid_col_bboxes, mask_reg_col_lines, [i for i in range(len(grid_col_bboxes))]):
        image_w_rl, last_grid_index_number = setBBoxesAutomatically(image_w_gcb, grid_col_bbox, annotate_mode, color, current_image_index + i, idx_increment, column_pos)
        last_grid_index_numbers.append(last_grid_index_number)
    current_image_index = max(last_grid_index_numbers) - idx_increment + 1 #reason? look on pg 584 in stewart's calculus 9th trans. ed.
...
def drawColumnLine(orig_image, image, boundary):
    ...
    if two_columns_boundary_set:
        image_w_column_line = cv2.line(image, column_line[0], column_line[1], NEON_GREEN, 2)
        ...
        im_bnd_gray = cv2.cvtColor(orig_image, cv2.COLOR_BGR2GRAY)
        _, im_thresh_otsu = cv2.threshold(im_bnd_gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        cv2.setMouseCallback("image", scanAndSetColumnLineCoordiates, [boundary])
        while True:
...
########################################################################################################################################################################################################
########################################################################################################################################################################################################
def SASOuterBoundaryAndMaskRegion(event, x, y, flags, param):
    global drawing
    ...
    boundary = param[0]
    if drawing_bbox_outer_boundary and drawing:
        ...
        if len(bboxes) == 0:
            if y > boundary[1][1]:
                ...
                scan_region = im_thresh_otsu[y - scan_radius:y + scan_radius, boundary[0][0]:boundary[1][0]]
                offset = -scan_radius
                reg_rows = {}
                cont_rows = []
                start = True
            ...
            if y > boundary[1][1]:
                ...
                scan_region = im_thresh_otsu[y - scan_radius:y + scan_radius, bb_x_start:bb_x_end]
                offset = -scan_radius
                reg_rows = {}
                cont_rows = []
                start = True
        ...
        if x < boundary[0][0]:
            mask_reg_col_line = [(boundary[0][0], y_start), (boundary[0][0], y_end)]
        elif x > boundary[1][0]:
            mask_reg_col_line = [(boundary[1][0], y_start), (boundary[1][0], y_end)]
        else:
            mask_reg_col_line = [(x, y_start), (x, y_end)]
            scan_radius = SCAN_RADIUS_MASK
            scan_region = im_thresh_otsu[boundary[0][1]:boundary[1][1], x - scan_radius:x + scan_radius]
            ...
            if idx is not None:
                ...
                if midpoint > boundary[1][0]:
                    mask_reg_col_line = [(boundary[1][0], y_start), (boundary[1][0], y_end)]
                elif midpoint < boundary[0][0]:
                    mask_reg_col_line = [(boundary[0][0], y_start), (boundary[0][0], y_end)]
                else:
                    mask_reg_col_line = [(midpoint, y_start), (midpoint, y_end)]
            else:
                #mask_reg_col_line = [(x, y_start), (x, y_end)]
                pass
    ...
    if event == cv2.EVENT_LBUTTONUP:
        drawing_bbox_outer_boundary = False
...
def binSearchContiguousWhiteRegions(rows, rows_indices, pixel_label_dict):
    if len(rows) > 1:
        pixel_label_dict = binSearchContiguousWhiteRegions(rows[:int(np.ceil(len(rows) / 2))], rows_indices[:int(np.ceil(len(rows) / 2))], pixel_label_dict)
        pixel_label_dict = binSearchContiguousWhiteRegions(rows[int(np.ceil(len(rows) / 2)):], rows_indices[int(np.ceil(len(rows) / 2)):], pixel_label_dict)
        return pixel_label_dict
    else:
        min_pixel_value = int(min(rows[0]))
        if min_pixel_value > 240:
...
def findMidpointOfLongestContWhiteReg(cont_white_regs):
    min_pixel_values = cont_white_regs.values()
    row_indices = cont_white_regs.keys()
    max_length = 0
    max_cont_rows = []
    cont_rows = []
    length = 0
    start = False
    for min_pixel_value, row_index in zip(min_pixel_values, row_indices):
        if min_pixel_value == 1:
            if start == False:
                start = True
...
def scanAndSetColumnLineCoordiates(event, x, y, flags, param):
    global drawing_column_line
    global column_line
    global boundary_left_column, boundary_right_column
    global x_start, y_start, x_end, y_end
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        if x > outer_boundary[1][0]:
            ...
            scan_region = im_thresh_otsu[outer_boundary[0][1]:outer_boundary[1][1], x - scan_radius:x + scan_radius]
            scan_region = np.transpose(scan_region)
            offset = -scan_radius
            reg_columns = {}
            cont_columns = []
    ...
    if drawing_grid_outer_boundary and drawing:
        if len(bboxes) == 0:
            if y > boundary[1][1]:
                ...
                scan_region = im_thresh_otsu[y - 20:y + 20, boundary[0][0]:boundary[1][0]]
                offset = -20
                reg_rows = {}
                cont_rows = []
                start = True
            ...
            if y > boundary[1][1]:
                ...
                scan_region = im_thresh_otsu[y - 20:y + 20, bb_x_start:bb_x_end]
                offset = -20
                cont_rows = []
                start = True
                for i in range(40):
        ...
        if x > boundary[1][0]:
            ...
            if grid_scan_mode is False:
                ...
                if len(grid_col_bboxes) == 0 and white_columns is None:
                    grid_region = im_thresh_otsu[boundary[0][1]:boundary[1][1], boundary[0][0]:boundary[1][0]]
...
# def OLDsetBBoxesAutomatically(image_w_new_bboxes, boundary, annotate_mode, color, column_pos=None):
#     global x_start, y_start, x_end, y_end
#     global current_image_index
#     global drawing
#     global label_ready
...
#     global header
#
#     mask_scan_region = im_thresh_otsu[y_start:y_end, x_start:mask_reg_col_line[0][0]]
#     temp_pos_y = binSearchForBBOXBoundaries("black", mask_scan_region, [i for i in range(y_start, y_end)], scan_up=False)
#     start_pos_y = binSearchForBBOXBoundaries("white", mask_scan_region[(temp_pos_y[1] - y_start):], [i for i in range(temp_pos_y[1], y_end)], scan_up=False)
#     if len(bboxes) == 0:
#         top_bbox_y = boundary[0][1]
#     else:
#         top_bbox_y = list(bboxes.values())[-1][1][1]
#     while True:
#         pos_y2 = binSearchForBBOXBoundaries("black", mask_scan_region[(start_pos_y[1] - y_start):], [i for i in range(start_pos_y[1], y_end)], scan_up=False)
#         if pos_y2[0] >= 240:
#             break
#         upper_scan_region = im_thresh_otsu[y_start:pos_y2[1], x_start:x_end]
#         temp_pos_y = binSearchForBBOXBoundaries("white", upper_scan_region, [i for i in range(y_start, pos_y2[1])], scan_up=True)
#         pos_y1 = binSearchForBBOXBoundaries("black", upper_scan_region[:(temp_pos_y[1] - y_start)], [i for i in range(y_start, temp_pos_y[1])], scan_up=True)
#         if pos_y1[0] >= 240:
#             break
...
#             masked_reg = cv2.addWeighted(im_region, 0.5, white_rect, 0.5, 1.0)
#             image_w_new_bboxes[mask[0][1]:mask[1][1], mask[0][0]:mask[1][0]] = masked_reg
#             cv2.rectangle(image_w_new_bboxes, mask[0], mask[1], color, 1)
#         if odds_only:
#             current_image_index += 2
#         else:
#             current_image_index += 1
#         cv2.rectangle(image_w_new_bboxes, bbox[0], bbox[1], color, 1)
#         x_0, y_0, x_1, y_1 = bbox[0][0], bbox[0][1], bbox[0][0], bbox[0][1]
#         l_x_0, l_y_0, l_x_1, l_y_1 = x_0 - 28, y_0, x_1 - 2, y_1 + 20
#         cv2.rectangle(image_w_new_bboxes, (l_x_0, l_y_0), (l_x_1, l_y_1), color, -1)
#         cv2.putText(image_w_new_bboxes, str(annotations[key]["index"]).zfill(3), (l_x_0 + 2, l_y_0 + 14), cv2.FONT_HERSHEY_SIMPLEX, 0.40, (255, 255, 255), 1, cv2.LINE_AA)
...
#         cv2.waitKey(1)
#         top_bbox_y = midpoint_y
#         start_pos_y = binSearchForBBOXBoundaries("white", mask_scan_region[(pos_y2[1] - y_start):], [i for i in range(pos_y2[1], y_end)], scan_up=False)
#     key = generateCode()
#     bbox = [(x_start, top_bbox_y), (x_end, y_end)]
#     bboxes[key] = bbox
#     addAnnotation(code=key, shape=SHAPE_BBOX, operation=annotate_mode, column_pos=column_pos, index=current_image_index)
